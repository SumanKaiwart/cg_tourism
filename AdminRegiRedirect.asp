...
<tr>
  ...
  <td width=100 align="center"><img src = "cm.jpg" height=50 width=90 style ="border-radius: 50%"><a href = "cm.html"><i style="font-size:11px;">Chief Minister of C.G. </i></a></td>
    ...
    objconn.open
	dim bolAlreadyExist
	if Request.Form("uname")= "" or Request.Form("gender") = "" or Request.Form("password")="" or Request.Form("dob") = "" or Request.Form("address")="" or Request.Form("email") = ""  or Request.Form("mobile") = "" then 
	Session("adminlogin")="abcd"
        response.Redirect("AdminRegister.asp")
	else
		bolAlreadyExist = false
		Set objrs= server.createObject("ADODB.RecordSet")
       objrs.open "admin", objconn, ,3 ,2
	   do while not (objrs.EOF or bolAlreadyExist)
	    if(Strcomp(objrs("aname"),Request.Form("aname"),vbTextCompare) = 0) then
		Response.write "Username already exists"
		bolAlreadyExist = true
		end if
		 if(Strcomp(objrs("apassword"),Request.Form("password"),vbTextCompare) = 0) then
		Response.write "<br><center>Password already exists </center>"%>
		<center><br><a href ="AdminRegister.asp">try again </a></center>
		<%bolAlreadyExist = true
		end if
		objrs.Movenext
	  Loop
	  ...
	  objrs.AddNew
	objrs("aname") = Request.Form("uname")
	objrs("agender") = Request.Form("gender")
	objrs("apassword") = Request.Form("password")
	objrs("adob") = Request.Form("dob")
	objrs("aaddress") = Request.Form("address")
	objrs("aemail") = Request.Form("email")
	objrs("amobile") = Request.Form("mobile")
	objrs.Update%>
	<br>
	<br>
	<center> <i>Thanks for registering</i> <br>
	For login <a href = "AdminLogin.asp">click here</a> 
	</center>
    <%
	end if
	objrs.Close
